...
| **TSAN-0014** | concurrent `list.sort()` of a shared `list` — `list_sort_impl` detaches the array with **no** critical section and `binarysort` rewrites it in place | low observed | take the list's per-object critical section in `list_sort_impl` | **not reproduced in isolation** (detach window ~3 instructions; fleet caught it once). Root cause grounded in current-main source. |
