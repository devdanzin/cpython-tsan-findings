...
| id | what | disposition |
|----|------|-------------|
| **TSAN-0053** | plain `dict` iterator: `dictiter_iternext_threadsafe`'s exhaustion path `fail: di->di_dict = NULL; Py_DECREF(d);` (`dictobject.c:6158-6159`) non-atomically drops the iterator's **one** owning ref to the dict, while the caller `dictiter_iternextkey:5791` reads `d = di->di_dict` unlocked → two `next()` threads both `Py_DECREF(d)` → **double-free / UAF / negative refcount** | **FILED → cpython#154130.** NEW crash, reproduced: ~8/8 SIGABRT (`_Py_NegativeRefcount` on the dict, `dictobject.c:6159`) on `debug-ft-nojit`; **SIGSEGV** on `release-ft-nojit-o0`. The **crash face** of **cpython#148873** (same iterator/repro/stack), which was **closed as a dup of gh-120496** (value-benign) — but gh-120496's own contract (colesbury/eendebakpt: "must **not crash** the interpreter") puts a double-free on the not-acceptable side. Function was built to be shared by **gh-112075/PR#115108** ("used from multiple threads simultaneously"); value-read made safe, exhaustion `Py_DECREF` left unguarded. Distinct from TSAN-0026 (value-benign `ma_values` race, same function). Covers keys/values/items. `_colorize` incidental (racing object = a plain `iter({...})`) |
