...
| `_colorize` | **RESOLVED via shrinkray → KNOWN = TSAN-0006 slow-mode residual = cpython#153981.** shrinkray reduced the 1313-line vehicle to 25 lines: the shared iterator was `itertools.count(10**18, 2)` next()+repr()'d by 4 threads (`_colorize`/weird-subclasses/managed-dict all incidental). = the big-int `long_cnt` repr **use-after-free** (`count_repr` borrows `lz->long_cnt`, `count_nextlong` replaces+returns it, `next()` caller frees it → SEGV in `PyObject_Repr`), the slow-mode residual #153908/#153917 left unfixed. Filed as **cpython#153981** (fix PR #153983), already corroborated by us. Small-int count = 0/5 (fixed); big-int = 5/5 on debug-ft + asan. |
