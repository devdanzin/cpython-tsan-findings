...
| **TSAN-0034** | interpreter finalization: `handle_thread_shutdown_exception` does `assert(interp->threads.head != NULL)` (`pylifecycle.c:3830`) — an **unlocked** read — on the line *before* `_PyEval_StopTheWorld`, racing an exiting thread's `tstate_delete_common` write (`pystate.c:1936`) which holds `HEAD_LOCK` | **NEW but LOW / debug-build only.** The racing read exists only inside the `assert` (`NDEBUG` removes it), so release builds cannot hit it. The function's comment ("we don't have to worry about locking this because the world is stopped") doesn't cover this line. **Not reproduced in isolation** (window = one unlocked pointer read vs a locked write; 0/14). No filing exists (verified via `gh api`). Fix: move the assert below the STW. Cataloged mainly for dedup hygiene |
