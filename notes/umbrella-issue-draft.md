...
| [TSAN-0006](https://gist.github.com/devdanzin/db21f2b29ab7572ce6c111b57b3cea5c) | `itertoolsmodule.c`: `count.__repr__` plain-reads `cnt` while `count_next` writes it with an **atomic CAS** — the writer was hardened, the reader missed | `_Py_atomic_load_ssize_relaxed` in `count_repr` | |
...
| [TSAN-0018](https://gist.github.com/devdanzin/6b99ef6dce08ac64d8d1d379308a8f86) | `dictobject.c`: readers of a shared dict's `dk_nentries` use plain loads while `setattr`/insert bump it atomically — including the **public `PyDict_Next` C-API** (via `_PyType_GetSubclasses`), so any extension iterating a shared dict is exposed. `LOAD_KEYS_NENTRIES` already exists at `:237` | `LOAD_KEYS_NENTRIES` at the reader sites (`_PyObject_IsInstanceDictEmpty`, `clear_lock_held`, `_PyDict_Next`) | |
