...
*`sys_addaudithook_impl` lazily initializes the per-interpreter audit-hook list (`interp->audit_hooks = PyList_New(0)`) with a plain, unlocked pointer store the first time any thread adds a hook. `should_audit` reads that same pointer on every audit event with a plain load. On a free-threaded build, the first-ever `sys.addaudithook()` store races with concurrent audit-event reads (and with sibling `addaudithook` stores). Unlike the C-level audit-hook list — guarded by `runtime->audit_hooks.mutex` since 2023 (gh-100227) — the Python-level `interp->audit_hooks` list (added 2021) got no equivalent guard when that lock and the free-threaded build landed: a provable incomplete migration. The memory-level race is certain (TSan-confirmed); its consequences are an empty-list leak on concurrent first-registration and, under a narrower interleaving, a silently dropped hook — a fail-open gap in the PEP 578 audit infrastructure.*
...
The asymmetry is the tell, and `git blame` dates it precisely as an **incomplete migration**:

- The *C-level* global hook list is a `struct { PyMutex mutex; struct _Py_AuditHookEntry *head; } audit_hooks` (`Include/internal/pycore_runtime_structs.h:264`). `PySys_AddAuditHook` serializes appends to it with `PyMutex_Lock(&runtime->audit_hooks.mutex)` (`:507`), and its unlocked reader carries the explicit "benign race" comment (`:292`–`:295`). **Both landed together in gh-100227 "Lock Around Adding Global Audit Hooks"** (commit `e822a676f1f3`, 2023-06; the mutex became a `PyMutex` for the free-threaded runtime in gh-111924, `cf6110ba1337`, 2023-12, by Sam Gross).
- The *Python-level* `interp->audit_hooks` lazy-init + append (`:539`–`:548`) dates to **commit `bcb094b41f7f` (Victor Stinner, 2021-02)** — it predates the locking work and was **never brought under `runtime->audit_hooks.mutex`** when the C path was hardened or when the free-threaded build made this field concurrently accessed. So the guard that already exists for audit-hook state simply isn't applied to the Python-level list.

The "benign race" comment therefore reasons specifically about the *C-level singly-linked list* — appending a node there leaves a reader either seeing it or not, "not [in] an inconsistent state." That reasoning does **not** cover `interp->audit_hooks`, where the racing operation is a *pointer store during lazy init* (and a Python-list append/iterate), not a link append.
...
- **Leaked list, and a fail-open dropped hook (security-relevant correctness).** The missing lock means the lazy init is more than a benign pointer race: two threads can both observe `interp->audit_hooks == NULL` at `:539` and both `PyList_New(0)` at `:540`, so one list wins the pointer and the other leaks. Because the `PyList_Append` at `:548` *re-reads* `interp->audit_hooks`, the common double-create outcome is that both threads append to the winning list and only an **empty list leaks** (no hook lost). A hook is actually **dropped** only under the narrower interleaving where one thread completes its append (`:548` into *its* list) *before* the other thread's store (`:540`) overwrites the field — orphaning the first thread's list *with its hook on it*. For PEP 578 auditing — used by security monitors that assume a successfully-returned `addaudithook()` means the hook is installed — that dropped hook is a monitoring gap that fails open silently. Empirically this drop is rare (a 256-thread barrier-synchronized first-registration stress did not lose a hook in 100 process runs — the same microscopic window that makes it hard to hit is why it slips through review), but the *memory* race that underlies it is certain and TSan-confirmed on every run.
...
Found by ThreadSanitizer fuzzing (`fusil --tsan`), fleet 01, vehicle `inst-03/python/sys-warning_threadsanitizer_data_race-tsanNEW`. This is the classic "unsynchronized lazy-init of interpreter-global state under free-threading" class; the existing `runtime->audit_hooks.mutex` shows the maintainers already lock the sibling C-level list, and `git blame` shows the Python-level list simply predates that work — a plausible oversight, not a deliberate "don't share" contract. It is genuinely interpreter-global state, so there is no per-object alternative a user could avoid sharing.

Issue search (2026-07-15): no existing report covers this race. `Python/sysmodule.c` is **unchecked** on the gh-116738 "Audit all built-in modules for thread safety" list, and the recent `sys.addaudithook` issues (#152912 / PR #152913) are about the hook's *exception* handling, not thread-safety. Confirmed still unfixed on current `main` (`heads/main:bcf98ddbc40`).
