...
The `fail:` path — `di->di_dict = NULL; Py_DECREF(d)` — was written for GIL-era Python (`git blame`: 2004 / 2010 / 2016), where only one thread ever runs `dictiter_iternext*` at a time, so clearing and dropping the reference is trivially safe. The lock-free `dictiter_iternext_threadsafe` fast path was added by **gh-112075 / PR #115108** ("Iterating a dict shouldn't require locks"), whose stated purpose is to "[handle] races against the dict as well as **allowing the iterator to be used from multiple threads simultaneously**." That PR made the *value read* safe (`_Py_TryIncrefCompare` / `acquire_key_value`) but **copied the old `fail: di->di_dict = NULL; Py_DECREF(d)` exhaustion path in unchanged** — so under `--disable-gil` two threads reach `fail:` concurrently and both drop the single owning reference. A read-modify-write on a shared `PyObject *` (`di_dict`) and its referent's refcount with no per-object locking, in the code path specifically built to be used from multiple threads.

## Prior art & the documented free-threading contract

This is **not** the value-benign iterator race that CPython documented and declined to fix — it is the crash that documentation explicitly excludes.

- **gh-112075** ("Make `dict` objects thread-safe in `--disable-gil` builds", colesbury) is the umbrella under which the lock-free dict iterator (`dictiter_iternext_threadsafe`, PR #115108) was written to be shared across threads. The double-free is a defect in that implementation, not an unsupported usage.
- **gh-120496** ("Sequence iterator thread-safety") reported that iterators "accessed with the same index" return duplicate/skipped values (tested on `range`/`list`/`dict`). The maintainers **decided not to fix the value problem** and instead documented it — the only thing that merged was a `Doc/glossary.rst` note (PR #120685/#120706); all three code-fix PRs (#120534/#120540/#120591) were closed unmerged. But the accepted contract was stated explicitly on that issue:
  - **eendebakpt:** "we want the container iterators to be thread-safe in the sense that iterating using multiple threads **will not crash the interpreter**, but it is ok to be not thread-safe in the sense that the iterator can return results that would under single-threading be considered a bug."
  - **colesbury:** "**Yes**."
  - eendebakpt also flagged the residual danger directly: "there is also the risks of **overflows inside the C code**."
- **gh-148873** reported *this* iterator's data race (the `di_dict` write) and was **closed as a duplicate of gh-120496** — i.e. filed under the value-benign class. That conflation is the gap: gh-148873's underlying defect is a **double-free**, and a crash is precisely what gh-120496 puts on the *not-acceptable* side of the line. The current code still double-frees.

So the finding sits squarely inside the maintainers' own "must not crash" bar (also gh-124397), in a dict iterator that gh-112075 built to be shared — while the *value* corruption it also causes is the acceptable, documented part.
...
- **Prior art — the data-race face was mis-closed as the value-benign class.** [cpython#148873](https://github.com/python/cpython/issues/148873) ("Possible data race in `dictiter_iternext` with free-threading build", **closed 2026-04-22 as a duplicate of gh-120496**) reported the *same* `di_dict` race with the *same* reproducer shape (a shared `iter(d)` driven by two `next()` threads) and the *same* call stack, pointing at the `di->di_dict = NULL` write racing the read. Closing it as a dup of the value-benign gh-120496 (see the section above) folds a **double-free** into the "duplicate/skipped values are acceptable" bucket — but gh-120496's own accepted contract is that iterators *must not crash the interpreter*. Current `main` still double-frees. This report supplies what #148873 lacked: proof it is a **crash** (negative refcount / UAF / SIGSEGV), with a reliable pure-Python reproducer — the same escalation that made the GenericAlias iterator ([cpython#154043](https://github.com/python/cpython/issues/154043)) and the struct `iter_unpack` iterator ([cpython#154013](https://github.com/python/cpython/issues/154013)) fileable. A fresh issue framed as a memory-safety crash, cross-referencing #148873 / gh-120496 / gh-112075, is the right move.
